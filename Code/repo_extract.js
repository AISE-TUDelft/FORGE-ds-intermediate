...
async function* searchReposByLicense(language, totalCount) {
  ...
  const init_query = licenses.map(license => `license:${license}`).join(' ');
  const query = `${init_query} language:${language}`
  try {
    let up = 900;
    let down = 850;
    while (true) { 
      ...
      if (intervalRepoC > 0 && intervalRepoC < 1000) {
        ...
        for (let date = 0; date <= dateArray.length - 2; date += 2) {
          for (let page = 1; page <= 10; page++) {
            ...
            if (up === 900) {
              response = await octokit.request("GET /search/repositories", {
                q: `${query} stars:>=900 created:${dateArray[date]}..${
                  dateArray[date + 1]
                }`,
                per_page: reposPerPage,
                page: page,
              ...
              response = await octokit.request("GET /search/repositories", {
                q: `${query} stars:${down}..${up} created:${dateArray[date]}..${
                  dateArray[date + 1]
                }`,
                per_page: reposPerPage,
                page: page,
...
async function saveRepos(languages, repoC) {
  ...
  try {
    ...
    for (lang of languages) {
      let counter = 0;
      filePathGlb = `./bigdataset_paper/${lang}CopyLeft.json`;
